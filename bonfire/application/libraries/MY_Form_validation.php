...
class MY_Form_validation extends CI_Form_validation
{
	...
	{
		$this->CI->form_validation->set_message('edit_unique', 'The value in &quot;%s&quot; is already being used.');

		// allow for more than 1 parameter
		$fields = explode(",", $params);

		// extract the first parameter
		list($table, $field, $current_id) = explode(".", $fields[0], 3);
		...
		if (isset($fields[1]))
		{
			...
			list($where_table, $where_field, $where_current) = explode(".", $fields[1], 3);
		...
		if ($query->row() && $row[$field] != $current_id)
		{
			return FALSE;
		}
		else
    ...
    /* Checks that a value is unique in the database
	 * untuk multi uniqe akan menggunakan jenis parameter [@nama_tabel, @nama_kolom.@variabel post,@nama_kolom.@variabel post,@nama_kolom.@variabel post, dstrsnya (mengulangi @nama_kolom.@variabel post)) ]
	 * i.e. '…|required|multi_unique[users,nama_jurusan.kode_jurusan,kode_jenjang.kode_jenjang, dan sterusnya]|trim…'
     * sebaiknya digunakan pada input dengan type ::submit
     */
     
    function multi_unique($value, $params)
	{
		$this->CI->form_validation->set_message('multi_unique', 'Data dengan inputan berikut sudah ada.');
		// allow for more than 1 parameter
		$fields = explode(",", $params);        
        
        $jumlah = count($fields);
        $kaye=array();
        $query = "select * from $fields[0] where  "; 
        for($i=1; $i<=$jumlah-1; $i++){
            $mau = explode('.', $fields[$i]);
            $saya .=  $mau[0]." ='". $this->CI->input->post($mau [1])."'" ;
            ($i<$jumlah-1) ? $saya .=' and ' :""; 
        }
        $query .= $saya;
    ...
    function edit_multi_unique($value, $params)
	{
		$this->CI->form_validation->set_message('edit_multi_unique', 'Data dengan inputan berikut sudah ada.');
		// allow for more than 1 parameter
		$fields = explode(",", $params);        
        ...
        list($field,$isi)=explode('.',$fields[1]);
